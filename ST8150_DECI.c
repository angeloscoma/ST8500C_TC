...
//* Módulo: Decisão														*
//* Função: Tomar decisão de acionamento								*
...
void valida_acionamentos(void){
	...
	if (aux_tempo_aciona == 0){
		...
		if (kvar_req > 0){								//Decisão sobre acionamento dos capacitores.
//			#if (com_rep == true)
//				if (modo_rep == 1){
//					if(kvar_req > kvar_req_anterior){	//Verifica se a carga reativa existente é capacitiva, mesmo com tarifação indutiva.
//						if (aux_tempo_aciona > 0){
//							return;
//						}
//					}
//					else{
//						kvar_req_anterior = kvar_req;
//					}
//				}
//			#endif
			#if (printar == true)
