...
#use rs232(baud = 2400, xmit=PIN_B1,  rcv=PIN_B4,RESTART_WDT,  parity=N, bits=8, errors, stream = rep)  
#use rs232(baud = 110, xmit=PIN_A0,  RESTART_WDT, parity=N, bits=8, errors, stream = sultech)  
...
const char cequipo[11] = "SREP-V1.00";
...
/* define funcionamento do temporizador 1 */
	...
	set_uart_speed (110, sultech);
	
/* define funcionamento da watchdog */
	time_out_rst = cequipo[ptrrx];			// somente para constar o nome e vers؋o no arquivo hex
