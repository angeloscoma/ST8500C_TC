...
reg_zero:
	...
	aux_w2 = read_word_nvr(ds_ptr_regs_high);				// 
	aux_l2 = (int32)make32(aux_w2,aux_w1);					// 
	...
	for (aux_b2 = 0; aux_b2 < aux_b1; aux_b2++){			// Movimentamos o ponteiro de escrita byte a byte.
		write_byte_flash(aux_l1++, buf_dados[aux_b2]);		// Escrevendo os valores já definidos no buffer.
	}
	WRDI(aux_l1);											// Desabilitamos flash para gravação.
	
	aux_l2++;												// Incrementamos o ponteiro de registro.
	aux_l3 = (limite_registros - 1);
	if(aux_l2 > aux_l3){
		aux_l2 = 0;
	}
	aux_w1 = (word)aux_l2;
	aux_w2 = (word)(aux_l2 >> 16);
	write_word_nvr(ds_ptr_regs, aux_w1);					// Salvamos o último valor do ponteiro e do
	write_word_nvr(ds_ptr_regs_high, aux_w2);				// Salvamos o último valor do ponteiro e do
	
	aux_b3 = (aux_l2 / nro_registros_por_banco);
	write_byte_nvr(ds_banco_escrita,aux_b3);				// banco de escrita.
