...
salva_regs:
	...
	aux_l1 = 0;
		...
		aux_b2 = read_byte_nvr(ds_banco_escrita);
	#else
		aux_w1 = read_word(ee_ptr_regs);
		aux_b2 = read_byte(ee_banco_escrita);
	#endif
	aux_l1 = (int32)(aux_b2);
...
void wr_falta_energia(void){
	...
	void calcula_autonomia(void){
		...
		// descobre a memoria disponível e exibe a quantidade disponível.
