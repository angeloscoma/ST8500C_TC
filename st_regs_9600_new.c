...
reg_zero:
	...
	aux_w1 = (word)read_word_nvr(ds_ptr_regs);			// Buscamos o ponteiro dentro do banco e o
	aux_b2 = (word)read_byte_nvr(ds_banco_escrita);		// banco de escrita atual.
	aux_l1 = (int32)(aux_w3 * aux_b2);					// O produto fornece a posição da linha de registro.
	...
	aux_l2 = (int32)(aux_l2 << 12);						// Antes de salvar, verificamos se o setor está apagado,
	aux_b1 = read_byte_flash(aux_l1);					// caso não esteja, apaga o setor de 4K.
	if (read_byte_flash(aux_l1) != 0xff){				// primeira posicao disponível sempre = 255.
	...
	if(aux_w1 > nro_registros_por_banco-1){				// Se o limite dentro do banco foi atingido,
		...
		write_word_nvr(ds_ptr_regs, 0);					// e reiniciamos o ponteiro.
		if(aux_b2 > nro_bancos-1){						// Se o limite de bancos foi atingido retornamos ao
			if(fila_circular == 0){						// verificamos o tipo de fila programado, no caso 
				registro = 0;							// de fila linear (circular = 0), encerramos os 
				prog_time = 0;							// processos relacionados ao registro de dados e 
				write_byte(ee_registro, registro);		// salvamos esta nova condição na EEPROM.
...
void calcula_autonomia(void){					// descobre a memoria disponível e exibe a quantidade disponível.
	aux_l1 = 0;
	aux_w2 = read_word_nvr(ds_ptr_regs);
	aux_w3 = read_word_nvr(ds_ptr_regs_high);
	aux_l1 = make32(aux_w3, aux_w2);
	if (modo_funca == 2){
